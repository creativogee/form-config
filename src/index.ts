		...
		sections: config.sections.map((section) => ({
			...
			items: section.items.map((item) => {
				const decomposedItem = Object.fromEntries(
					Object.entries(item).filter(([key]: [keyof Item, unknown]) =>
						allow.includes(key)
					)
				) as Partial<Item>;
...
export function stage<T extends Record<string, any>>(
...
export const getChangeGroup =
	...
	(value: any) => {
		...
		if (group[item.name]) {
			if (isObject(value)) {
				if (
					group[item.name].some((groupItem: any) => deepEqual(groupItem, value))
				) {
					setGroup({
						...group,
						[item.name]: group[item.name].filter(
							(groupItem: any) => !deepEqual(groupItem, value)
						),
					});
				} else {
					setGroup({
...
export function unprepare(config: Config): Record<string, any> {
	const unprepareItem = (item: Item, acc: Record<string, any>) => {
	...
	const unprepareSection = (section: Section, acc: Record<string, any>) => {
		section?.items?.forEach((item) => {
			unprepareItem(item, acc);
		});
