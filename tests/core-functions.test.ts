...
describe('Core Functions', () => {
    ...
    it('should include value field when specified in allow options', () => {
      const configWithEntries: Config = {
        ...
        sections: [
          {
            ...mockConfig.sections[0]!,
            items: [{ ...mockConfig.sections[0]!.items![1]!, entry: 'option1' }],
          },
        ],
      };

      const result = decompose(configWithEntries, { allow: ['name', 'entry', 'value'] });
        ...
        sections: [
          {
            ...mockConfig.sections[0]!,
            items: [
              { ...mockConfig.sections[0]!.items![0]!, default: 'default text' },
            ...
            items: [
              {
                ...
                entry: 'parent value',
                subItems: [
                  { name: 'subItem1', entry: 'sub value 1', type: 'text' as const },
  ...
  describe('translate', () => {
    it('should translate labels using provided function', () => {
      ...
      const t = (key: string) => translationMap[key as keyof typeof translationMap] || key;
  ...
  describe('getChangeGroup', () => {
    ...
    it('should remove existing object value from group', () => {
      const objectValue = { id: 1, name: 'test' };
      const group: Record<string, any[]> = { testField: [objectValue, { id: 2, name: 'test2' }] };
      const setGroup = jest.fn();
      const item = { name: 'testField', type: 'group' as const };
